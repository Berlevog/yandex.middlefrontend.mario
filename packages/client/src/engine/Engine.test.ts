...
describe("Должен вернуть углы прямоугольника которые соприкасаются с другим прямоугольником", () => {
	...
	it("topLeft", function() {
		const result = { ...noCollision, topLeft: true, isCollision: true, left: true, top:true };
		expect(getCollision(new Rect({ x: 38, y: 38, width: 10, height: 10 }), rect)).toEqual(result);
	});
	it("topMiddle", function() {
		const result = { ...noCollision, topMiddle: true,isCollision: true,  top:true };
		expect(getCollision(new Rect({ x: 0, y: 30, width: 60, height: 60 }), rect)).toEqual(result);
	});
	it("topRight", function() {
		const result = { ...noCollision, topRight: true, isCollision: true, right: true, top:true };
		expect(getCollision(new Rect({ x: -25, y: 25, width: 60, height: 60 }), rect)).toEqual(result);
	});
	it("leftMiddle", function() {
		const result = { ...noCollision, leftMiddle: true, isCollision: true, left: true };
		expect(getCollision(new Rect({ x: 30, y: 0, width: 60, height: 60 }), rect)).toEqual(result);
	});
	it("bottomRight", function() {
		const result = { ...noCollision, bottomRight: true, isCollision: true, right: true, bottom:true };
		expect(getCollision(new Rect({ x: -30, y: -30, width: 60, height: 60 }), rect)).toEqual(result);
	});
	it("bottomMiddle", function() {
		const result = { ...noCollision, bottomMiddle: true, isCollision: true, bottom:true };
		expect(getCollision(new Rect({ x: 0, y: -30, width: 60, height: 60 }), rect)).toEqual(result);
	});
	it("bottomLeft", function() {
		const result = { ...noCollision, bottomLeft: true, isCollision: true, left: true, bottom:true };
		expect(getCollision(new Rect({ x: 30, y: -30, width: 60, height: 60 }), rect)).toEqual(result);
	});
	it("leftSide", function() {
		const result = { ...noCollision, bottomLeft: true, topLeft: true, leftMiddle: true, bottom:true, isCollision:true, left:true, top:true };
